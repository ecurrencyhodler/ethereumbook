...
The word _token_ derives from the Old English "tacen" meaning a sign or symbol. Commonly used to mean privately-issued coin-like items that have insignificant value such as transportation tokens, laundry tokens, and arcade tokens.
...
The most obvious use of tokens is as digital private currencies. However, this is only one possible use. Tokens can be programmed to serve many different, often overlapping, functions. For example, a token can simultaneously convey a voting right, an access right, and ownership of a resource. Currency is just the first "app".  Below is a list of the many different ways Tokens can be used:
